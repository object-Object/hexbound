...
uniform float shake_block_count = 21f;
...
void main() {
    ...
    if (mod(Hexbound_WorldTime, shake_repeat_ticks) < shake_length_ticks) {
        fixed_uv.x += (random((trunc(texCoord0.y * shake_block_count) / shake_block_count) + mod(Hexbound_WorldTime, 200f)) - 0.5) * shake_power_U;
